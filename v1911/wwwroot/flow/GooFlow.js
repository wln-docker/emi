...
function GooFlow(bgDiv, property) {
	...
	if (property.haveHead) {
		...
		if (property.headLabel) {
			tmp += "<label title='" + (property.initLabelText || "未命名") + "' "
				+ (GooFlow.prototype.color.main ? "style='background:" + GooFlow.prototype.color.main + "'" : "") + ">" + (property.initLabelText || "未命名") + "</label>";
		}
		for (var x = 0; x < property.headBtns.length; ++x) {
			let text = '';
			switch (property.headBtns[x]) {
				case "save": text = '保存流程设计'; break;
				case "rename": text = '修改名称'; break;
				case "reload": text = '重新载入'; break;
				case "new": text = '清空设计'; break;
				case "back": text = '返回'; break;
			}
			tmp += "<a href='javascript:void(0)' class='GooFlow_head_btn'><i class='ico_" + property.headBtns[x] + "'></i><font>" + text +"</font></a>";
		}
		tmp += "</div>";
		this.$head = $(tmp);
		...
		this.onBtnBackClick = null;//返回按钮定义
		this.onRenameClick = null;//重命名流程按钮定义
		...
		if (property.headBtns)
			this.$head.on("click", { inthis: this }, function (e) {
				...
				} else if (tar.tagName === "FONT") {
					tar = tar.parentNode.childNodes[0];
				}
				...
				switch ($(tar).attr("class")) {
					...
					case "ico_back": if (This.onBtnBackClick !== null) { This.onBtnBackClick(); } else { history.back(); } break;
					case "ico_undo": This.undo(); break;
					case "ico_redo": This.redo(); break;
					case "ico_rename": if (This.onRenameClick !== null) This.onRenameClick(); break;
